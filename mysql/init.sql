...
INSERT INTO checklist_items (category_id, title, description, order_num) VALUES
(2, 'Check for duplicate registration/Overwrite existing user', '## Objective\nPrevent users from creating multiple accounts or overwriting existing accounts.\n\n## Test Cases\n- Register with same email twice\n- Register with username that already exists\n- Check if system allows duplicate accounts\n\n## Expected Result\nApplication should reject duplicate registrations with clear error message.', 1),
(2, 'Check for weak password policy', '## Password Requirements\nCheck if application enforces:\n- Minimum length (8+ characters recommended)\n- Complexity (uppercase, lowercase, numbers, special chars)\n- Dictionary checking\n- Historical password check\n\n## Test\n```\nTry registering with passwords like: "123", "admin", "password"\n```', 2),
(2, 'Check for reuse existing usernames', '## Vulnerability\nAllowing registration with already taken usernames can enable account takeover.\n\n## Testing\n1. Create user "testuser"\n2. Attempt to register "testuser" again\n3. Verify system rejects with unique username error', 3),
(2, 'Check for insufficient email verification process', '## Verification Methods\n- Confirmation link sent to email\n- OTP validation\n- Email domain validation\n\n## Gaps to Test\n- No verification required\n- Verification link doesn\'t expire\n- Link can be bypassed\n- Any domain accepted (including disposable)', 4),
(2, 'Weak registration implementation-Allows disposable email addresses', '## Risk\nDisposable emails enable spam accounts and evade restrictions.\n\n## Test\n```\nRegister with:\n- 10minutemail.com\n- guerrillamail.com\n- mailinator.com\n- temp-mail.org\n```\n\n## Mitigation\nMaintain blocklist of known disposable email providers.', 5),
(2, 'Weak registration implementation-Over HTTP', '## Security Issue\nRegistration over HTTP exposes:\n- Credentials in transit (sniffable)\n- MITM attacks possible\n- Password harvesting\n\n## Check\n1. Access registration page\n2. Verify URL is HTTPS\n3. Check for mixed content warnings', 6),
(2, 'Overwrite default web application pages by specially crafted username registrations', '## Privilege Escalation\nRegistering username like "admin", "root", "system" might grant access to special paths.\n\n## Test\n```\nTry registering with:\n- admin\n- administrator\n- root\n- /admin\n- ../admin\n- admin/\n```\n\n## Verify\nCheck if special usernames get special permissions.', 7);
-- Inserir items de Session Management Testing
INSERT INTO checklist_items (category_id, title, description, order_num) VALUES
(3, 'Identify actual session cookie out of bulk cookies in the application', '## Method\nUse browser DevTools to inspect all cookies set by application.\n\n## What to Look For\n- JSESSIONID\n- PHPSESSID\n- ASP.NET_SessionId\n- Custom session identifiers\n\n## Tools\n- Browser F12 → Application → Cookies\n- Burp Suite Proxy\n- OWASP ZAP', 1),
(3, 'Decode cookies using some standard decoding algorithms such as Base64, hex, URL, etc', '## Decoding Techniques\n- **Base64**: Use `echo value | base64 -d`\n- **Hex**: Online hex decoders\n- **URL Encoding**: Decode %20 → space, etc.\n- **JWT**: jwt.io for token inspection\n\n## Risk\nDecoded tokens may contain sensitive user information.', 2),
(3, 'Modify cookie.session token value by 1 bit/byte', '## Test Session ID Strength\n1. Get valid session cookie\n2. Change last character\n3. Use modified cookie\n4. If accepted = weak validation\n\n## Example\n```\nOriginal: sessionid=abc123def456\nModified: sessionid=abc123def457\n```\n\n## Expected\nModified cookie should be rejected.', 3),
(3, 'If self-registration is available and you can choose your username, log in with a series of similar usernames', '## Privilege Escalation\nTest if username variations grant different permissions.\n\n## Examples\n- testuser → testuser1\n- admin_user → admin\n- regular_user → regular-user\n\n## Check\n- Do similar usernames have different permissions?\n- Can we escalate privileges via username manipulation?', 4),
(3, 'Check for session cookies and cookie expiration date/time', '## Cookie Attributes to Verify\n- **Max-Age**: Session timeout duration\n- **Expires**: Absolute expiration time\n- Idle timeout\n- Absolute timeout\n\n## Best Practice\n- 15-30 minutes for sensitive operations\n- Longer for read-only sessions\n- Logout should invalidate immediately', 5),
(3, 'Identify cookie domain scope', '## Domain Scope Testing\nCheck Set-Cookie header for Domain attribute.\n\n## Issues to Find\n- `.example.com` → accessible from subdomains (sometimes bad)\n- No Domain set → browser default (more secure)\n- Overly permissive domain scope\n\n## Vulnerability\nCookie accessible from unintended subdomains.', 6),
(3, 'Check for HttpOnly flag in cookie', '## Security Feature\nHttpOnly flag prevents JavaScript access to cookie (blocks XSS cookie theft).\n\n## Check\n```\nIn browser console:\nconsole.log(document.cookie);\n```\n\n## Expected\nSession cookie should NOT appear (HttpOnly is set).', 7),
(3, 'Check for Secure flag in cookie if the application is over SSL', '## SSL/TLS Protection\nSecure flag ensures cookie only transmitted over HTTPS.\n\n## Test\n1. Load site over HTTPS\n2. Check cookie has Secure flag\n3. Try accessing over HTTP\n4. Verify cookie not sent over HTTP', 8),
(3, 'Check for session fixation i.e. value of session cookie before and after authentication', '## Session Fixation Attack\n1. Get session ID before login (unauthenticated)\n2. Log in\n3. Compare session ID before/after login\n\n## Expected Behavior\nSession ID MUST change after authentication.\\n\n## Vulnerable\nIf same ID used pre/post auth = Session Fixation vulnerability.', 9),
(3, 'Replay the session cookie from a different effective IP address or system', '## Session Binding Test\n1. Get valid session cookie\n2. Use cookie from different IP/device\n3. Check if session is still valid\n\n## Expected\nSession should be tied to IP or user-agent to prevent replay attacks.', 10),
(3, 'Check for concurrent login through different machine/IP', '## Concurrent Session Test\n1. Log in from Machine A\n2. Log in from Machine B simultaneously\n3. Check if both sessions are valid\n\n## Risk\nAllowing concurrent logins enables account sharing and makes audit trails unreliable.', 11),
(3, 'Check if any user pertaining information is stored in cookie value or not', '## Information Disclosure\nExamine cookie values for:\n- User ID\n- Username\n- Email\n- Role/Permissions\n- Sensitive flags\n\n## Risk\nInformation in plaintext cookies is exposed to XSS and CSRF attacks.', 12),
(3, 'Failure to Invalidate Session on (Email Change,2FA Activation)', '## Account Changes\nWhen user changes:\n- Email address\n- Phone number\n- 2FA settings\n- Password\n\nSystem should invalidate all existing sessions.\n\n## Risk\nOld sessions could be used for unauthorized access if security params change.', 13);

-- Inserir items de Authentication Testing
INSERT INTO checklist_items (category_id, title, description, order_num) VALUES
(4, 'Username enumeration', '## Vulnerability\nApplication reveals whether username exists through different responses.\n\n## Testing Methods\n1. Test login with valid/invalid usernames\n2. Check response messages\n3. Measure response times\n4. Check forgot password responses\n\n## Examples\n- "User not found" vs "Invalid password"\n- Timing differences in responses', 1),
(4, 'Bypass authentication using various SQL Injections on username and password field', '## SQL Injection in Auth\n```\nUsername: admin\' --\nPassword: anything\n\nQuery becomes: SELECT * FROM users WHERE username=\'admin\'--\'\n```\n\n## Common Payloads\n- `admin\' or \'1\'=\'1\n- `\' OR 1=1 --\n- `admin\'#\n- `\' UNION SELECT NULL,NULL--`', 2),
(4, 'Lack of password confirmation on Change email address', '## Vulnerability\nChanging email without password verification allows attackers to lock out user.\n\n## Test\n1. Logged in as user\n2. Try to change email\n3. If no password required = vulnerability\n\n## Impact\nAccount takeover via email change.', 3),
(4, 'Lack of password confirmation on Change password', '## Critical Check\nPassword change MUST require current password.\n\n## Test\n1. Log in\n2. Go to change password\n3. Provide only new password\n4. If accepted without old password = CRITICAL\n\n## Exploit\nXSS/CSRF could force password change on victim account.', 4),
(4, 'Lack of password confirmation on Manage 2FA', '## 2FA Bypass\n2FA settings changes should require password/2FA verification.\n\n## Test\n1. Enabled 2FA\n2. Try to disable without password\n3. Try to change phone number\n\n## Risk\nAttacker could disable 2FA and take over account.', 5),
(4, 'Is it possible to use resources without authentication? Access violation', '## Broken Access Control\nTest if authenticated endpoints accessible without login.\n\n## Examples\n- `/admin` without authentication\n- `/api/users/123` without token\n- Direct file access `/uploads/sensitive.pdf`\n\n## Test\nRemove authentication tokens and test all endpoints.', 6),
(4, 'Check if user credentials are transmitted over SSL or not', '## Encryption in Transit\n1. Login attempt\n2. Check Network tab (F12)\n3. Verify HTTPS/TLS used\n4. Verify no plaintext transmission\n\n## Risk\nCredentials in HTTP can be intercepted via MITM.', 7),
(4, 'Weak login function HTTP and HTTPS both are available', '## Mixed Protocol\nIf login available over both HTTP and HTTPS:\n1. HTTP version vulnerable to downgrade attack\n2. Credentials sniffable on HTTP\n\n## Test\nAccess login page via HTTP and verify it redirects to HTTPS.', 8),
(4, 'Test user account lockout mechanism on brute force attack', '## Rate Limiting\nAfter N failed attempts, account should be temporarily locked.\n\n## Test\n1. Try 10 wrong passwords\n2. Check if account locked\n3. Check lockout duration\n\n## Bypass\nAttackers need unlimited attempts otherwise.', 9),
(4, 'Bypass rate limiting by tampering user agent to Mobile User agent', '## Rate Limit Bypass\nSome systems have different limits for mobile/desktop.\n\n## Test\n```\nChange User-Agent header to:\nMozilla/5.0 (iPhone; CPU iPhone OS 14_0 like Mac OS X)\n```\n\nIf rate limiting resets = vulnerability.', 10),
(4, 'Bypass rate limiting by tampering user agent to Anonymous user agent', '## User-Agent Bypass\nSome systems track by User-Agent for rate limiting.\n\n## Test\n1. Generate failed login attempts\n2. Change User-Agent to random value\n3. If you can continue = rate limit bypass', 11),
(4, 'Bypass rate liniting by using null byte', '## Null Byte Injection\nSome parsers treat `%00` as string terminator.\n\n## Example\n```\nPOST /login\nusername=admin%00admin@fake.com\npassword=test\n```\n\nMight be parsed as just "admin".', 12),
(4, 'Create a password wordlist using cewl command', '## Custom Wordlist Generation\n```\ncewl https://target.com -w wordlist.txt -d 3\n```\n\nCEWL crawls site and extracts words for password cracking.\n\n## Use Cases\n- Brute force specific to target\n- Likely password combinations\n- Company-specific terminology', 13),
(4, 'Test Oauth login functionality', '## OAuth 2.0 Testing\nTest OAuth integration for security flaws:\n- Missing state parameter\n- Invalid redirect_uri\n- Scope escalation\n- Token expiration', 14),
(4, 'OAuth: Resource Owner → User', '## OAuth Terminology\n**Resource Owner**: The end-user (you)\\n\nYou own your data and authorize applications to access it.', 15),
(4, 'OAuth: Resource Server → Twitter', '## OAuth Terminology\n**Resource Server**: Service holding protected resources (your tweets, profile)\\n\nHosted on Twitter servers.', 16),
(4, 'OAuth: Client Application → Twitterdeck.com', '## OAuth Terminology\n**Client Application**: The app requesting access\\n\nTwitterdeck wants to access your Twitter account.', 17),
(4, 'OAuth: Authorization Server → Twitter', '## OAuth Terminology\n**Authorization Server**: Handles user authentication and token issuance\\n\nTwitter provides this service.', 18),
(4, 'OAuth: client_id → Twitterdeck ID', '## OAuth Terminology\n**client_id**: Public identifier for application\\n\nUnique ID assigned to Twitterdeck by Twitter.', 19),
(4, 'OAuth: client_secret → Secret Token', '## OAuth Terminology\n**client_secret**: Confidential token (like password)\\n\nShould NEVER be exposed in client-side code.', 20),
(4, 'OAuth: response_type → Defines the token type', '## OAuth Terminology\n**response_type**: Parameter specifying what to return\\n\n- `code`: Authorization code flow (most secure)\n- `token`: Implicit flow (legacy, less secure)', 21),
(4, 'OAuth: scope → The requested level of access', '## OAuth Terminology\n**scope**: Permissions requested\\n\nExamples:\n- `read:tweets`\n- `write:profile`\n- `email`', 22),
(4, 'OAuth: redirect_uri → The URL user is redirected to', '## OAuth Terminology\n**redirect_uri**: Where user sent after authorization\\n\nMust be registered and validated. Vulnerability if not validated properly.', 23),
(4, 'OAuth: state → Main CSRF protection', '## OAuth Terminology\n**state**: Random token for CSRF protection\\n\nIncluded in redirect, verified by client to ensure same user.', 24),
(4, 'OAuth: grant_type → Defines the grant_type', '## OAuth Terminology\n**grant_type**: How client obtains access token\\n\n- `authorization_code`\n- `client_credentials`\n- `password`\n- `refresh_token`', 25),
(4, 'OAuth: code → The authorization code', '## OAuth Terminology\n**code**: Temporary code from auth server\\n\nExchanged for access_token. Should expire quickly (5-10 min).', 26),
(4, 'OAuth: access_token → The token to make API requests', '## OAuth Terminology\n**access_token**: Bearer token for API access\\n\nSent in Authorization header. Shouldn\'t be stored in localStorage (XSS risk).', 27),
(4, 'OAuth: refresh_token → Allows new access_token', '## OAuth Terminology\n**refresh_token**: Long-lived token\\n\nUsed to get new access_token when expired. Must be stored securely.', 28),
(4, 'OAuth Code Flaws: Re-Using the code', '## Authorization Code Reuse\n**Vulnerability**: Using same authorization code twice.\n\n## Test\n1. Get authorization code\n2. Exchange for token (succeeds)\n3. Try exchanging same code again\n\n## Expected\nSecond attempt should fail.', 29),
(4, 'OAuth Code Flaws: Code Predict/Bruteforce and Rate-limit', '## Code Predictability\nIf authorization codes are sequential or predictable:\n\n## Test\n1. Get multiple authorization codes\n2. Analyze pattern\n3. Try bruteforcing codes\n4. Check for rate limiting\n\n## Expected\nCodes should be random, high-entropy, rate-limited.', 30),
(4, 'OAuth Code Flaws: Is the code for application X valid for application Y?', '## Code Sharing Between Apps\n**Vulnerability**: Code from App A works with App B.\n\n## Test\n1. Get auth code for App A\n2. Try using it with App B\n\n## Expected\nCode should be app-specific and fail if used for wrong app.', 31);
